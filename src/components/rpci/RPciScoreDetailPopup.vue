...
const props = defineProps({
  ...
  initialPosition: { type: Object as PropType<{ top?: string; left?: string; right?: string; bottom?: string }>, default: () => ({ top: '9%', left: '16px' }) }
})
    ...
    <div class="flex justify-between items-center p-3 border-b border-gray-200 bg-gray-100/5 select-none"
      ...
      <div class="flex items-center gap-2">
        ...
        <span v-if="typeof titleData.status === 'number'">
          <p class="text-xs font-semibold px-2 py-0.5 rounded-full text-center w-25"
        ...
        <thead class="sticky top-0 bg-white z-10">
          <tr>
            <th v-for="header, index in tableHeaders" :key="`header-${index}`"
              ...
              <div class="flex items-center justify-center gap-1"> <span>{{ header.text }}</span>
                <button v-if="header.filterable" type="button"
                  ...
                  @click.stop="emitFilter(header.key)" aria-label="Filter column">
                </button>
        ...
        <tbody>
          ...
          <tr v-for="(row, rowIndex) in paginatedData" :key="`row-${rowIndex}`" class="hover:bg-gray-200">
            <td v-for="(header, colIndex) in tableHeaders" :key="`cell-${rowIndex}-${colIndex}`"
              ...
              :class="{ 'text-center': colIndex === tableHeaders.length - 1 || header.key === 'score' }"> <template
                v-if="header.key === 'score' && typeof row[header.key] === 'number'">
    ...
    <div v-if="totalPages > 1" class="p-3 border-t border-gray-200 bg-white flex justify-center items-center gap-1">
      ...
      <template v-for="(page, index) in displayedPageNumbers" :key="`page-${index}`">
        <button v-if="typeof page === 'number'" type="button"
          ...
          :class="{ 'bg-blue-500 text-white hover:bg-blue-600 border-blue-500 cursor-default': page === currentPage, 'bg-white text-gray-700 border-gray-300': page !== currentPage }"
