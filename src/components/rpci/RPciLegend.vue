...
<template>
  <div class="floating-top-right fixed top-[24px] z-[2] border-1 border-gray-200 rounded-sm">
    ...
    <!-- 펼친 상태 레전드 -->
    <div v-else class="bg-white rounded-sm rounded-tl-none shadow p-4 space-y-2 w-55 relative transition-all">
      <!-- 닫기 버튼 -->
      <button
      ...
      <!-- 등급 리스트 -->
      <div v-for="(grade, idx) in grades" :key="idx" class="flex items-center justify-between px-0 space-x-5">
        <div class="w-full">
          <p class="text-sm font-semibold px-3 py-1 rounded-full text-center"
