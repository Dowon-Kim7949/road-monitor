...
const props = defineProps({
  modelValue: { // Supports v-model
    type: String,
    required: true,
  },
  radioGroupName: { // Unique name for the radio group
    type: String,
...
function getCategoryClass(category) {
  ...
  if (catLower.includes('fail') || catLower.includes('serious') || catLower.includes('very poor') || catLower.includes('poor')) {
    return 'text-red-600';
