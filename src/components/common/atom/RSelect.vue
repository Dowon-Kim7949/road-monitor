...
const findNextEnabledIndex = (startIndex: number): number => {
  ...
  const currentEnabledIndex = enabledOptions.value.findIndex(
    (opt) => opt.value === props.options[startIndex]?.value,
  )

  let nextEnabledIndex: number
  if (currentEnabledIndex === -1) {
    // 현재 선택된 것이 없거나 비활성 상태면 첫번째 활성 옵션 선택
    nextEnabledIndex = 0
  } else {
...
const findPreviousEnabledIndex = (startIndex: number): number => {
  ...
  if (currentEnabledIndex === -1) {
    // 현재 선택된 것이 없거나 비활성 상태면 마지막 활성 옵션 선택
    previousEnabledIndex = enabledOptions.value.length - 1
  } else {
    previousEnabledIndex =
      (currentEnabledIndex - 1 + enabledOptions.value.length) % enabledOptions.value.length // 이전 인덱스 (순환)
  }
...
const focusPreviousOption = () => {
  let prevIndex =
    focusedOptionIndex.value >= 0 ? focusedOptionIndex.value - 1 : props.options.length - 1 // 현재 포커스가 없으면 마지막부터 시작
  while (prevIndex >= 0) {
...
const scrollOptionIntoView = () => {
  if (
    ...
    focusedOptionIndex.value < props.options.length
  ) {
    const focusedElement = optionsListElement.value.children[
      focusedOptionIndex.value
    ] as HTMLLIElement
    if (focusedElement) {
