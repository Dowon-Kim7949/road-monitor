  ...
  <div class="space-y-2 flex flex-col overflow-y-auto min-h-[300px] max-h-[300px]">
    ...
    <div class="flex flex-1 flex-col overflow-y-auto pr-1">
      ...
      >
        ...
        <img
          v-if="type === 'road'"
          :src="item.src || 'https://via.placeholder.com/120x72?text=No+Image'"
          alt="history"
        ...
        <div class="text-sm text-gray-800 whitespace-nowrap">
          <template v-if="type === 'rpci'">
            ...
            <div class="flex items-center space-x-3 text-sm">
              <span
                class="text-sm font-semibold px-2 py-1 rounded-b-full rounded-t-full w-30 text-center"
                :style="{
              ...
              <span
                ...
                :style="{ color: getLevelDetailsByScore(item.score!)?.color }"
              >
                {{ item.score }}점
              </span>
    ...
    <div class="flex justify-center py-2">
      <RButton
        v-if="full"
        type="tertiary"
        class="w-30 rounded-b-full rounded-t-full hover:bg-gray-100 hover:text-white"
        size="small"
        icon="chevron-up"
        :label="t('button.collapse')"
        icon-pos="right"
        @click="$emit('collapse')"
      />
      <RButton
